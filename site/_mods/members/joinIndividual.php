...
switch ($STAGE) {
    ...
    case "application":
        ...
        $js->script('js', '', '$(document).ready(function(){$("#membersJoinIndividualForm").validate();});');
        ...
        $main->_div();
        $main->hx(2, 'Members\' - Join as an Individual Member', '', '');
        $main->_hx(2);
      ...
      <p>This version of the subscription renewal form must only be used by those who wish to pay by <strong>Credit or Debit Card</strong>. The payment service is provided by PayPal, but does not require a PayPal account.</p>');
        ...
        // Institutional or Institutional (Agency)
        ...
        $main->_select();
        $js->script('js', '', 'var description = new Array(); ' . $descriptionArray . '$(document).ready(function(){$("#membershipType").change(function(){message_index = $("#membershipType").val(); $("#membershipTypeDescription").empty(); if (message_index > 0) {$("#membershipTypeDescription").append(description[message_index]); }});});');

        $main->br(1);
        $main->add(' - ');
        $main->span('membershipTypeDescription', '');
        $main->add($membershipDescription);
        $main->_span();
        $main->_td();
        $main->_tr();
        $main->_tbody();
        ...
        $main->tr('', '');
        $main->td('', '', '', '');
        $main->add('Tick the box if you DO NOT want to receive Newsletters, Council Reports, and other News by email from the society:');
        $main->_td();
        $main->td('', '', '', '');
        !isset($_SESSION['application']['newsletter']) ? null : $applicationNewsletter = $_SESSION['application']['newsletter'];
        if ($applicationNewsletter) {
        ...
        $main->tr('', '');
        $main->td('', '', '', '');
        $main->add('Tick the box if you DO NOT consent for your details being passed on to other members of the Society for purposes of palaeontological research:');
        $main->_td();
        $main->td('', '', '', '');
        ...
        $main->_table();
        // Data Protection
        $main->hx(4, 'Data Protection', '', '');
        $main->_hx(4);
        $main->table('', 'membersTable');
        $main->tbody('', '');
        $main->tr('', '');
        $main->td('', '', '', '');
        $main->add('<strong>I AGREE</strong> to consent for The Palaeontographical Society to hold the details provided in our card and database records so that we can keep our records accurate and up to date. Your details will not be passed to any third party without your consent and will be held for administrative purposes provided you remain a member.');
        $main->_td();
        $main->td('', '', '', '');
    ...
    case "process":
        ...
        if ($db->sql_numrows($result_member) != 0) {
            ...
            // Add Javascript
            $js->script('js', __SITEURL . '_js/jquery/jq.ext/validation/jquery.validate.min.js', '');
            $js->script('js', '', '$(document).ready(function(){$("#membersJoinIndividualEmailForm").validate();});');
            ...
            $main->add('<a href="' . __SITEURL . 'members/" title="Link: Return to Members Login">Return to Members Login</a>');
            $main->_div();
            $main->hx(2, 'Members\' - Join as an Individual Member - Review', '', '');
            $main->_hx(2);
            ...
            $main->add('<br /> - ');
            $main->span('membershipTypeDescription', '');
            $main->add($row_selection['mod_members_membership_Description']);
            $main->_span();
            $main->_td();
            $main->_tr();
            $main->_tbody();
            ...
            if ($_SESSION['application']['type'] != 'Student') {
                ...
                $js->script('js', '', '$(document).ready(function(){$("#checkoutReviewForm").validate({rules: {checkoutTerms: {required: true}}});});');
                $main->form('?membership=individual&amp;stage=transfer', 'POST', '', 'checkoutReviewForm', '');
                $main->div('shopTemplateCheckoutTerms', '');
                $main->hx(4, 'Terms and Conditions of Membership', '', '');
                $main->_hx(4);
    ...
    case "interlude":
        ...
        $main->add('<a href="' . __SITEURL . 'members/" title="Link: Return to Members Login">Return to Members Login</a>');
        $main->_div();
        $main->hx(2, 'Members\' - Join as an Individual Member - Interlude', '', '');
        $main->_hx(2);
        $main->div('membersJoinIndividualInterlude', '');
        $main->add('<p>Thank you for starting the student membership application process.</p><p>You will shortly receive an email explaining the next steps to take.</p>');
        $main->_div();
    ...
    case "continue":
        ...
        if (($db->sql_numrows($order_result) == 0) OR ($row_order['mod_members_application_OrderStatus'] != 'AwaitingPayment')) {
            ...
            $main->add('<a href="' . __SITEURL . 'members/" title="Link: Return to Members Login">Return to Members Login</a>');
            $main->_div();
            $main->hx(2, 'Members\' - Join as an Individual Member - Continue', '', '');
            $main->_hx(2);
            $main->div('membersJoinIndividualReview', '');
            $main->add('<p>Thank you for continuing the student membership application process. Please check the information below before proceeding to the PayPal payment site.</p>');
            ...
            $main->tr('', '');
            $main->td('', '', '', '');
            $main->add('I DO NOT want to receive Newsletters, Council Reports, and other News by email from the society:');
            $main->_td();
            $main->td('', '', '', '');
            ...
            $main->tr('', '');
            $main->td('', '', '', '');
            $main->add('I DO NOT consent for your details being passed on to other members of the Society for purposes of palaeontological research:');
            $main->_td();
            $main->td('', '', '', '');
    ...
    case "transfer":
        ...
        $js->script('js', '', '$(document).ready(function(){setTimeout(function() {$("#membersPayPalTranfer").submit();}, 5000);});');
    ...
    case "student-transfer":
        ...
        if (($db->sql_numrows($order_result) == 0) OR ($row_order['mod_members_application_OrderStatus'] != 'AwaitingPayment')) {
            ...
            $main->div('shopTemplateCheckoutTransferForm', '');
            $main->br(2);
            $main->add('Please wait while your application/order is transferred to PayPal...');
            $main->br(2);
            $main->add('<!-- PayPal Logo --><img  src="https://www.paypal.com/en_US/i/bnr/horizontal_solution_PPeCheck.gif" alt="PayPal Logo"><!-- PayPal Logo -->');
            $main->br(1);
            $main->img(__SITEURL . 'image/shop/paypalTransfer.gif', '', '', 'Image: PayPal Transfer', 'Image: PayPal Transfer');
            $main->br(2);
            $main->add('If you are not automatically transferred please press the button below:');
            $main->br(2);
    ...
    case "pdt":
        ...
        $main->hx(2, 'Members\' - Join as an Individual Member - Confirmation', '', '');
        $main->_hx(2);
        $main->_div();
                ...
                if ($NO_ERROR) {
                    ...
                    if ($row_order['mod_members_application_PaymentStatus'] != 'Completed') {
                        if ($keyarray['payment_status'] == 'Pending') {
                            $db->sql_query("UPDATE mod_members_application SET mod_members_application_PaymentStatus='{$keyarray['payment_status']}', mod_members_application_PaymentStatusInfo='{$keyarray['pending_reason']}' WHERE mod_members_application_InvoiceID='{$keyarray['invoice']}'");
                            ...
                            $db->sql_query("UPDATE mod_members_application SET mod_members_application_PaymentStatus='{$keyarray['payment_status']}', mod_members_application_PaymentStatusInfo='{$keyarray['reason_code']}' WHERE mod_members_application_InvoiceID='{$keyarray['invoice']}'");
                        } elseif ($keyarray['payment_status'] == 'Completed') {
                            $sql = "UPDATE mod_members_application SET mod_members_application_PaymentStatus='{$keyarray['payment_status']}', mod_members_application_PaymentStatusInfo='Awaiting Account Activation', mod_members_application_OrderStatus='PaymentReceived' WHERE mod_members_application_InvoiceID='{$keyarray['invoice']}'";
                            $db->sql_query($sql);
                        } else {
                            $db->sql_query("UPDATE mod_members_application SET mod_members_application_PaymentStatus='{$keyarray['payment_status']}', mod_members_applicationPaymentStatusInfo='' WHERE mod_members_application_InvoiceID='{$keyarray['invoice']}'");
                        }
                    }
                    ...
                    $main->hx(4, 'Payment Details', '', '');
                    $main->_hx(4);
                    $main->table('', 'checkoutTable');
                    $main->tbody('', '');
                    $main->tr('', '');
                    $main->td('', 'title', '', '');
                    $main->add('Order/Invoice N&deg;:');
                    $main->_td();
                    $main->td('', '', '', '');
    ...
    case "ipn":
        ...
        if (!$fp) {
            // HTTP ERROR
            $db->sql_query("INSERT INTO mod_members_ipn (mod_members_ipn_Date, mod_members_ipn_Status, mod_members_ipn_Info) VALUES ('$date','HTTP ERROR','')");
        } else {
            fputs($fp, $header . $req);
            while (!feof($fp)) {
                $res = fgets($fp, 1024);
                if (strcmp($res, "VERIFIED") == 0) {
                    ...
                    if ($NO_ERROR) {
                        // log success
                        $db->sql_query("INSERT INTO mod_members_ipn (mod_members_ipn_Date, mod_members_ipn_Status, mod_members_ipn_Info) VALUES ('$date','SUCCESS','$req')");
                        ...
                        switch ($_POST['payment_status']) {
                            case 'Completed':
                                ...
                                $db->sql_query("UPDATE mod_members_application SET mod_members_application_PaymentStatus='{$_POST['payment_status']}', mod_members_application_PaymentStatusInfo='Awaiting Account Activation', mod_members_application_OrderStatus='PaymentReceived' WHERE mod_members_application_InvoiceID='{$_POST['invoice']}'");
                            ...
                            case 'Failed':
                                ...
                                $db->sql_query("UPDATE mod_members_application SET mod_members_application_PaymentStatus='{$_POST['payment_status']}', mod_members_application_PaymentStatusInfo='Do Not Activate - Order Canceled', mod_members_application_OrderStatus='Canceled' WHERE mod_members_application_InvoiceID='{$_POST['invoice']}'");
                            ...
                            case 'Reversed':
                                ...
                                $db->sql_query("UPDATE mod_members_application SET mod_members_application_PaymentStatus='{$_POST['payment_status']}', mod_members_application_PaymentStatusInfo='Do Not Activate - Membership Refunded. {$_POST['reason_code']}', mod_members_application_OrderStatus='Refunded' WHERE mod_shop_orders_InvoiceID='{$_POST['invoice']}'");
                            ...
                            case 'Processed':
                                // Log but no email - payment accepted waiting for it to clear
                                $db->sql_query("UPDATE mod_members_application SET mod_members_application_PaymentStatus='{$_POST['payment_status']}', mod_members_application_PaymentStatusInfo='Payment is Processing', mod_members_application_OrderStatus='AwaitingPayment' WHERE mod_members_application_InvoiceID='{$_POST['invoice']}'");
                                break;

                            case 'Canceled_Reversal':
                                // Log but no email - payment refund cancled/returned (this would be a manaual problem to solve)
                                // Get reason form reason_code POST
                                $db->sql_query("UPDATE mod_members_application SET mod_members_application_PaymentStatus='{$_POST['payment_status']}', mod_members_application_PaymentStatusInfo='Manual Processing Required. {$_POST['reason_code']}', mod_members_application_OrderStatus='PaymentReceived' WHERE mod_members_application_InvoiceID='{$_POST['invoice']}'");
                            ...
                            case 'Pending':
                                ...
                                $db->sql_query("UPDATE mod_members_application SET mod_members_application_PaymentStatus='{$_POST['payment_status']}', mod_members_application_PaymentStatusInfo='{$_POST['pending_reason']}', mod_members_application_OrderStatus='AwaitingPayment' WHERE mod_members_application_InvoiceID='{$_POST['invoice']}'");
                    ...
                    // log for manual investigation
                    $db->sql_query("INSERT INTO mod_members_ipn (mod_members_ipn_Date, mod_members_ipn_Status, mod_members_ipn_Info) VALUES ('$date','INVALID','$res')");
                }
            }
            fclose($fp);
        }
