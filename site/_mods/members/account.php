...
$main = new xhtml;
$main->hx(2,'Member\'s Account','','');$main->_hx(2);
$main->p('','');
...
$main->div ('membersAccount','');
  $main->hx(3,'My Subscription','','');$main->_hx(3);
  ...
  $main->hx(3,'Options','','');$main->_hx(3);
  // Three Columns
  $main->div('membersAccountCols','yui-gb');
    $main->div('membersAccountCol1','yui-u first');
      $main->hx(4,'My Details','','');$main->_hx(4);
      $main->ul('','');
        $main->li('<a href="?mode=myDetails&amp;view=username" title="Link: Change Username">Change Username</a>','','');$main->_li();
        $main->li('<a href="?mode=myDetails&amp;view=password" title="Link: Change Password">Change Password</a>','','');$main->_li();
        $main->li('<a href="?mode=myDetails&amp;view=personal" title="Link: Edit Personal Details">Edit Personal Details</a>','','');$main->_li();
        //$main->add('No Links');
      $main->_ul();
    $main->_div();
    $main->div('membersAccountCol2','yui-u');
      $main->hx(4,'My Subscription','','');$main->_hx(4);
      $main->ul('','');
        if (date("Y") >= (int)($row_members['mod_members_users_SubYear'])) {
          $main->li('<a href="'.__SITEURL.'members/renew/?membership=individual" title="Link: Renew Subscription">Renew Subscription</a>','','');$main->_li();
        } else {
          $main->li('<span class="notActive">Renew Subscription</span>','','');$main->_li();
        }
