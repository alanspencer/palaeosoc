...
while ($row_orders = $db->sql_fetchrow($result_orders)) {
    ...
    $main->tr('', '');
    $main->td('', '', '', '');
    $main->add($row_orders['mod_shop_orders_InvoiceID']);
    $main->_td();
    $main->td('', '', '', '');
    $main->add($day . '/' . $month . '/20' . $year . ' ' . $hour . ':' . $minute . ':' . $second);
    $main->_td();
    $main->td('', '', '', '');
    $main->add($row_orders['mod_shop_orders_OrderStatus']);
    $main->_td();
    $main->td('', '', '', '');
    $main->add($numberItems);
    $main->_td();
    $main->td('', '', '', '');
    $main->add(money_format('%n', $num->round2DP($row_orders['mod_shop_orders_ItemTotal'] + $row_orders['mod_shop_orders_Postage'])));
    $main->_td();
    $main->td('', '', '', '');
    $main->add('<a href="?mode=shop&amp;view=order&amp;id=' . $row_orders['mod_shop_orders_InvoiceID'] . '&amp;returnURL=' . $_GET['view'] . '" title="View Order: ' . $row_orders['mod_shop_orders_InvoiceID'] . '">View Order</a>');
    $main->_td();
    $main->_tr();
}
$main->_tbody();
