...
$main->_div();
$main->hx(2,'Members\'s Account - Change Password','','');$main->_hx(2);
$main->p('','');
$main->add('Allows the alteration of your password.');
$main->_p();
$main->div ('membersChangePassword','');
  $main->hx(3,'Change Password','','');$main->_hx(3);
  $main->p('','');
  ...
  $main->tbody('', '');
  $main->tr('', '');
  $main->td('', '', '', '2');$main->add('<em>The password must only contain alfa-numerical characters.</em><br /><br />Your Password Strength is: <span id="passwordResult"><span class="passwordBad">Too short - must be at least 5 characters long.</span></span>');$main->_td();
  $main->_tr();
  $main->tr('', '');
  $main->td('', 'title', '', '');$main->add('<label for="password1">New Password:</label>');$main->_td();
  $main->td('', '', '', '');
  ...
  $main->tr('', '');
  $main->tr('', '');
  $main->td('', 'title', '', '');$main->add('<label for="password2">Re-type New Password:</label>');$main->_td();
  $main->td('', '', '', '');
