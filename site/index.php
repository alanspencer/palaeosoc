...
function emailMessageCreator($MESSAGE_ID, $VAR_ARRAY, $db){
  ...
  $iop_email_message_header = array();
  $contenttype = NULL;
  $boundary = NULL;
...
function emailMailer($MESSAGE_ID, $TO, $CC, $BCC, $FROM, $VAR_ARRAY, $db) {      
  ...
  $__allRecipients = array();
  //---- To Headers
  if($TO != null) {
    $isFirst = true;
    if(is_array($TO)) {
      foreach ($TO as $__key => $__val) {
        if (is_array($val)) {
          if (!$isFirst) {
            $headers['To'] .= '; ';
          }
...
if ($MYSQLDB_ERROR) {
  ...
  $o->div('doc4','yui-t2');
    $o->div('hd',''); // Start Header
      ...
      $o->_div();        
      // H1 Title
      $o->hx (1, 'The Palaeontographical Society', '', '');$o->_hx(1);
    $o->_div(); // End Header
    $o->div('bd',''); // Start Body
      $o->add('<a id="pageNavigation" name="pageContent"></a>');
      $o->div('navigation','yui-b');
        // Navigation Block
        $o->hx(2,'Navigation','','hidden');$o->_hx(2);
